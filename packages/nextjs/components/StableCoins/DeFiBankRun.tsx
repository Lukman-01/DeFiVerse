...
import { AlertTriangle, ArrowDown, Banknote, Users } from "lucide-react";
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg ${
              isSimulating ? "bg-gray-200 cursor-not-allowed" : "bg-blue-500 hover:bg-blue-600 text-white"
            }`}
          >
            {isSimulating ? "Simulating..." : "Start Simulation"}
          </button>
        </div>
        <p className="mt-2 text-gray-600">Visualizing the stages and impact of a DeFi bank run event</p>
      </div>
          ...
          {stages.map(stage => (
            <div
              ...
              className={`p-4 rounded-lg border transition-all ${
                currentStage >= stage.id ? "border-red-500 bg-red-50" : "border-gray-200"
              }`}
            >
              ...
              {currentStage >= stage.id && <p className="text-sm text-red-600 mt-2">Impact: {stage.impact}</p>}
            </div>
          ))}
        </div>
