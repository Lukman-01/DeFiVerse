...
import { ChevronRight, Cpu, DollarSign, Shield } from "lucide-react";
...
const StablecoinTypes: React.FC = () => {
  ...
  const stablecoinTypes: StablecoinTypesMap = {
    ...
    collateral: {
      ...
      mechanism:
        "Users lock cryptocurrency as collateral to mint stablecoins, typically requiring over-collateralization.",
      collateralRatio: "150%+",
    },
    algorithmic: {
  ...
  const stabilityData: StabilityDataPoint[] = [
    { day: 1, reserve: 1.0, collateral: 1.01, algorithmic: 0.98 },
    { day: 2, reserve: 1.0, collateral: 0.99, algorithmic: 1.03 },
    { day: 3, reserve: 1.0, collateral: 1.02, algorithmic: 0.95 },
    { day: 4, reserve: 1.0, collateral: 0.98, algorithmic: 1.04 },
    { day: 5, reserve: 1.0, collateral: 1.01, algorithmic: 0.97 },
  ];
  ...
  return (
    <div className="p-6 bg-white rounded-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        {(Object.entries(stablecoinTypes) as [keyof typeof stablecoinTypes, StablecoinTypeDetails][]).map(
          ([key, type]) => (
            <button
              key={key}
              onClick={() => setSelectedType(key as typeof selectedType)}
              className={`p-4 rounded-lg border transition-all ${
                selectedType === key ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-blue-300"
              }`}
            >
              <div className="flex items-center space-x-3">
                ...
                <div className="text-left">
                  ...
                  <p className="text-sm text-gray-500">{type.examples.join(", ")}</p>
                </div>
              </div>
            </button>
          ),
            ...
            <div className="h-64">
              <ResponsiveContainer>
                <LineChart data={stabilityData}>
                  ...
                  <Tooltip />
                  <Line type="monotone" dataKey={selectedType} stroke={getStrokeColor(selectedType)} strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            </div>
